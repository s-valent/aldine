...
export default defineConfig({
  ...
  webServer: process.env.ALDINE_AUTH_URL ? undefined : {
    command: 'npm run build -w apps/web && PORT=3200 AUTH_ENABLED=1 ALDINE_RESET_ECHO=1 TRUST_PROXY=1 RL_REGISTER_BURST=200 DATA_DIR=$(pwd)/.data-auth META_DIR=$(pwd)/.secrets-auth npx tsx apps/server/src/index.ts',
    cwd: '..',
    port: 3200,
